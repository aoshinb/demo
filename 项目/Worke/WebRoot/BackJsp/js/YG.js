...
layui.use('table', function(){*/
	...
	var tableIns = table.render({
		elem: '#test'
			,url:'http://localhost:8080/Worke/Servlet?action=seleYG'
	...
	,title: '用户数据表'
		,cols: [[
			...
			{field: 'juese', title: '角色',  align:'center'},
			{field: 'bumen', title: '部门', minWidth:150, align:'center'},
	...
	function upUser(userid){//修改用户   echoYG
		layui.layer.open({
			title : "修改用户信息",
			type : 2,
			content : "http://localhost:8080/Worke/common/pop/YGupdate.jsp",
			area:['400px','450px'],
			success:function(layero, index){
				$.ajax({
					url:"http://localhost:8080/Worke/Servlet?action=echoYG",
					type:"post",
					data:{"id":userid},
	...
	function deleUser(userid){//删除用户
		alert("删除"+userid);
		$.ajax({
			url:"http://localhost:8080/Worke/Servlet?action=deleteYG",
			data:{"id":userid},
			type:"post",
			success:function(data){
				if(data == "1"){
	...
	function addUser() {//新增用户
		layui.layer.open({
			title : "添加用户",
			type : 2,
			content : "http://localhost:8080/Worke/common/pop/YGadd.jsp",
			area:['400px','450px']
		})
	}
});
